/* VENDORED COPY — real source of truth is PARENA/runtime/parena_runtime.h.
 * Copied here (not referenced by relative path) so PITVIPER's own build
 * doesn't take a hard filesystem dependency on PARENA being cloned
 * alongside it — same tradeoff any vendored dependency makes. Written
 * from a git-worktree-isolated agent (S192) that could not write to or
 * reference PARENA's own checkout directly. Keep in sync by hand if
 * parena_runtime.h changes; no automated sync exists yet.
 */
